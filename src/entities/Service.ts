...
@Index("uq_service_code", ["code"], { unique: true }) //OVO JE DODATO
@Entity("service", { schema: "iivt_2025" })
export class Service {
  @PrimaryGeneratedColumn({ type: "int", name: "service_id", unsigned: true })
  serviceId: number;

  //OVO ISPOD ZA CODE JE DODATO
  ...
  @ManyToOne(() => User, (user) => user.createdServices, { //OVO JE IZMENJENO
    onDelete: "CASCADE",
    onUpdate: "CASCADE",
  })
  ...
  @ManyToOne(() => User, (user) => user.updatedServices, { //OVO JE IZMENJENO
    onDelete: "CASCADE",
    onUpdate: "CASCADE",
  })
